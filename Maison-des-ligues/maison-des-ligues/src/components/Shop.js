...
function Shop() {
    ...
    useEffect(() => {
        ...
        axios.get('http://192.168.1.17:3000/api/produitsroute/produit')
            .then(response => {
    ...
    const addToCart = async (product) => {
        try {
            ...
            const response = await fetch('http://192.168.1.17:3000/api/produitsroute/ajout', {
                method: 'POST',
                headers: {
            ...
            <div className='liste'>
                {products.map(product => (
                    <div key={product.pid} className="box-product">
                        {product.image && <img src={`http://192.168.1.17:3000/${product.image}`} alt={product.name} style={{ maxWidth: '50%' }} />}
