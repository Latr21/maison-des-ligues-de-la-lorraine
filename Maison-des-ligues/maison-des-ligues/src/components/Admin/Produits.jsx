...
const Produits = () => {
    ...
    useEffect(() => {
        fetch("http://192.168.1.17:3000/api/produitsroute/produit")
    ...
    const handleAddProduct = async () => {
        ...
        try {
            const response = await fetch('http://192.168.1.17:3000/api/produitsroute/produit', {
                method: 'POST',
                body: formDataToSend,
            });
    ...
    const handleUpdateProduct = async () => {
        ...
        try {
            const response = await fetch(`http://192.168.1.17:3000/api/produitsroute/produit/${selectedProduct.pid}`, {
                method: 'PUT',
                body: formDataToSend,
            });
    ...
    const handleDeleteProduct = async (pid) => {
        try {
            const response = await fetch(`http://192.168.1.17:3000/api/produitsroute/produit/${pid}`, {
                method: 'DELETE',
            });
            ...
            <div className='liste'>
                {products.map(product => (
                    <div key={product.pid} className="box-product">
                        {product.image && <img src={`http://192.168.1.17:3000/${product.image}`} alt={product.name} style={{ maxWidth: '100%' }} />}
